...
function control(){
    setTimeout(() => {
        ...
        if (b == 6) {
            ...
            if (turn == 0) {
                run(turponu, b, turnpo, (turn + 1))
                dispalyturn.innerHTML="Red Turns"
                turn = 1;
            }
            else {
                run(turponu, b, turnpo, (turn + 1))
                dispalyturn.innerHTML="Green's turn"
                turn = 0;
            }
        }
    }, 501);
...
function run(turponu, b, turnpo, turn) {
    ...
    if (sum == 80) {
        document.getElementsByClassName("another")[0].innerHTML = `<div class="gameover"> GAME OVER ..... <div class="winner"> ${winner} will won this game </div></div>`
        ...
        if (turponu == 2) {
            document.getElementById(`player${turn}`).parentElement.innerHTML = `<div class="button2" id="player${turnoppo}"></div>`
